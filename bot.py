...
def sanitize(text: str) -> str:
    """Remove characters outside the Basic Multilingual Plane that Telegram rejects."""
    return re.sub(r"[^\u0000-\uFFFF]", "", text)


def now_wib() -> datetime:
    """Current datetime in WIB (GMT+7), timezone-aware."""
...
async def save_reminder(chat_id: int, remind_at: datetime, message: str):
    """remind_at must be timezone-aware (WIB)."""
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO reminders (chat_id, remind_at, message) VALUES ($1, $2, $3)",
            chat_id, remind_at, message
        )
...
async def list_reminders(chat_id: int) -> list:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT id, remind_at, message FROM reminders WHERE chat_id=$1 AND sent=FALSE ORDER BY remind_at ASC",
            chat_id
        )
    return rows
...
async def reminder_checker():
    """Background loop: fires due reminders every 30 seconds."""
    await asyncio.sleep(5)  # let DB init finish
    while True:
        try:
            async with db_pool.acquire() as conn:
                ...
                for row in rows:
                    text = sanitize(f"hey! pengingat kamu nih: {row['message']}")
...
def parse_remind_datetime(time_str: str) -> datetime | None:
    """
    Parse user-supplied time string into a timezone-aware WIB datetime.
    Supported formats:
      besok 08:00        tomorrow at HH:MM WIB
      lusa 14:30         day after tomorrow
      10/03 09:00        DD/MM this year
      10/03/2026 09:00   DD/MM/YYYY
      09:30              today if future, else tomorrow
    ...
    try:
        if time_str.startswith("besok "):
            hm = time_str[6:].strip()
            h, m = map(int, hm.split(":"))
            target_date = today + timedelta(days=1)
        elif time_str.startswith("lusa "):
            hm = time_str[5:].strip()
            h, m = map(int, hm.split(":"))
            target_date = today + timedelta(days=2)
        elif re.match(r"\d{1,2}/\d{1,2}/\d{4} \d{2}:\d{2}", time_str):
            date_part, time_part = time_str.split(" ")
            d, mo, y = map(int, date_part.split("/"))
            h, m = map(int, time_part.split(":"))
            target_date = datetime(y, mo, d).date()
        elif re.match(r"\d{1,2}/\d{1,2} \d{2}:\d{2}", time_str):
            date_part, time_part = time_str.split(" ")
            d, mo = map(int, date_part.split("/"))
            h, m = map(int, time_part.split(":"))
            target_date = datetime(today.year, mo, d).date()
        ...
        dt = datetime(target_date.year, target_date.month, target_date.day, h, m, tzinfo=WIB)
        if re.match(r"\d{2}:\d{2}$", time_str) and dt <= now_wib():
            dt += timedelta(days=1)
        return dt
    except Exception:
        return None
...
# --- Vent detection ---
VENT_KEYWORDS = [
    "stress", "stres", "capek banget", "lelah", "exhausted", "overwhelmed",
    "nangis", "sedih banget", "mau nangis", "pengen nangis",
    "gak kuat", "nggak kuat", "ga kuat", "nggak sanggup", "ga sanggup",
    "nyerah", "give up", "hopeless", "putus asa", "down banget", "bad day",
    "hari yang berat", "berat banget", "susah banget", "stuck"
]


def is_venting(text: str) -> bool:
    tl = text.lower()
    return any(kw in tl for kw in VENT_KEYWORDS)


VENT_SYSTEM_ADDON = (
    "\n\nPENTING: User ini sedang curhat atau vent. Ubah mode kamu jadi pendengar yang hangat dan empatik. "
    "Jangan langsung kasih solusi -- duluin validasi perasaan mereka dulu. "
    "Respond dengan lembut, penuh pengertian, kayak sahabat yang beneran peduli. "
    "Pakai kalimat kayak 'ih yang beneran?', 'aduh itu berat banget', 'wajar kamu ngerasa gitu', "
    "'aku denger kamu', 'cerita dong lebih'. Jangan toxic positivity."
)
...
async def webhook(request: Request):
    ...
    if text.startswith("/remind "):
        ...
        if "|" not in body:
            await send_message(
                ...
                "contoh: /remind besok 08:00 | kumpul tugas kimia\n"
                "atau: /remind 10/03 14:00 | deadline proyek"
            )
            return {"ok": True}
        ...
        if remind_dt is None:
            await send_message(
                chat_id,
                "hm, format waktunya gak aku kenali. coba:\n"
                "besok 08:00 | ..., lusa 14:30 | ..., 10/03 09:00 | ..., atau 09:30 | ..."
            )
            return {"ok": True}
        await save_reminder(chat_id, remind_dt, msg_part.strip())
        wib_str = remind_dt.strftime("%d %b %Y, %H:%M WIB")
        await send_message(chat_id, f"oke! aku ingetin kamu pada {wib_str}")
        return {"ok": True}
    ...
    if text == "/reminders":
        ...
        if not rows:
            ...
            lines = []
            for r in rows:
                dt_wib = r["remind_at"].astimezone(WIB)
                lines.append(f"- {dt_wib.strftime('%d %b %Y, %H:%M WIB')}: {r['message']}")
        ...
        explain_messages = [
            {
                "role": "system",
                "content": (
                    "Kamu adalah guru yang menjelaskan topik dengan cara yang sederhana, menarik, dan mudah dipahami. "
                    "Gunakan analogi, contoh nyata, dan bahasa yang casual tapi akurat. "
                    "Strukturkan penjelasan dengan jelas: konsep utama, cara kerja, contoh nyata. "
                    "Bahasa Indonesia yang gaul dan relatable untuk pelajar muda."
                )
            },
            {"role": "user", "content": f"Jelaskan: {topic}"}
        ]
        try:
            explanation = await ask_groq(explain_messages)
            await send_message(chat_id, explanation, reply_to=message_id)
        except Exception as e:
            await send_message(chat_id, f"aduh gagal jelasinnya: {e}")
        return {"ok": True}
        ...
        essay_messages = [
            {
                "role": "system",
                "content": (
                    "Kamu adalah asisten penulisan esai akademik. "
                    "Untuk topik yang diberikan, buat: "
                    "1) Outline esai (pendahuluan, 3 argumen utama, kesimpulan) "
                    "2) Paragraf pembuka yang kuat "
                    "3) Tips untuk mengembangkan setiap bagian. "
                    "Gunakan bahasa Indonesia yang formal tapi tetap mengalir."
                )
            },
            {"role": "user", "content": f"Tolong bantu buat esai tentang: {prompt_text}"}
        ]
        try:
            essay_help = await ask_groq(essay_messages)
            await send_message(chat_id, essay_help, reply_to=message_id)
        except Exception as e:
            await send_message(chat_id, f"aduh gagal ya: {e}")
        return {"ok": True}
